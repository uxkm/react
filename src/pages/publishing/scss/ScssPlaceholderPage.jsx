...
function ScssPlaceholderPage() {
  ...
  if (!findPublishingPage("scss", category, slug)) {
    return <Doc404Page />;
  }
  ...
  return (
    <>
      <PageMeta
        ...
        description={`${getPublishingSectionTitle("scss", category)} 문서 페이지입니다. React 구조로 변환 진행 상태를 안내합니다.`}
        keyword={`${getPublishingSectionTitle("scss", category)}, SCSS, ${slug}`}
        layout={{ mainClass: "scss_page", type: "publishing" }}
      />
      <>
        ...
        <section>
          <h2>소스 기준 경로</h2>
          <pre>
            <code>{`src/html/publishing/scss/${category}/${slug}.njk`}</code>
          </pre>
        </section>

        <p>
          <Link to="/publishing/scss">SCSS 목록으로 돌아가기</Link>
        </p>
      </>
    </>
  );
...
export default ScssPlaceholderPage;
