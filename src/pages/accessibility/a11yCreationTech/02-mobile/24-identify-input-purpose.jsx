...
const PAGE_LAYOUT = {
  mainClass: "identify-input-purpose",
  type: "accessibility",
  titleSticky: true,
  info: false,
  note: true,
...
const PAGE_DATA = {
  title: "입력 목적 식별 | 모바일 접근성 콘텐츠 제작 기법(\"인식의 용이성(Perceivable)\")",
  description: "모바일 접근성 콘텐츠 제작 기법이란 장애 유무에 관계없이 모든 사용자가 모바일 콘텐츠를 쉽게 이해하고 이용할 수 있도록 하는 설계하는 방법입니다. 이 페이지에서는 MACAG ‘입력 목적 식별’에 따라, 필드가 어떤 종류의 정보를 받는지 OS·브라우저에 알려 자동완성과 입력 보조를 쓸 수 있게 하는 방법을 정리합니다.",
  keyword: "Accessibility, a11y, macag, WCAG, 입력 목적, autofill, textContentType, importantForAutofill, 인식의 용이성",
}

function IdentifyInputPurposePage() {
  ...
  return (
    <>
      	...
      	<blockquote className="uk_note mt_xxl" role="note">
      		...
      		<p>
      			<b className="t_black">관련 지침(적응성) : 입력 항목의 목적(이름·이메일·전화번호 등)을 프로그램적으로 식별 가능하게 제공했는가?</b><br />
      			브라우저·OS의 자동완성, 비밀번호 관리자, 입력 보조 기능이 필드 목적을 알아야 사용자가 반복 입력 부담을 줄이고 오류를 낮출 수 있습니다.
      		</p>
      		<p className="mt_ms reference">
      			<a href="/accessibility/a11y/04-a11yCag/03-macag" className="fas weight-500">모바일 앱 접근성 체크리스트(MACAG) 전체 보기</a><br />
      			<a href="https://www.w3.org/WAI/WCAG22/quickref/#identify-input-purpose" title="새창 열림" target="_blank" rel="noreferrer" className="fas weight-500">WCAG 2.2 — Identify Input Purpose</a>
      		</p>
      	</blockquote>
      	...
      	<section className="indent mt_xxl">
      		...
      		<p className="mt_l">
      			모호한 입력란은 인지 부하를 높이고, 모션 제한이 있는 사용자에게는 치명적일 수 있습니다.
      			각 필드가 어떤 종류의 데이터를 받는지 OS에 알려 주면 키보드 유형, 자동완성, 보안 정책이 맞물립니다.
      		</p>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst info_lst gap-column-important-m">
      			<li><strong>운동·세밀 조작이 어려운 사용자</strong><p>자동완성으로 타이핑 횟수를 줄일 수 있는 사용자.</p></li>
      			<li><strong>인지·기억 장애인</strong><p>‘무엇을 넣어야 하는지’를 시스템이 안내해 주는 것이 도움이 되는 사용자.</p></li>
      			<li><strong>저시력·고령 사용자</strong><p>적절한 키보드(숫자·이메일)가 곧바로 열리면 오입력이 줄어드는 사용자.</p></li>
      			<li><strong>전체 사용자</strong><p>비밀번호 관리자·OTP 자동 입력 등 일상적 편의를 기대하는 사용자.</p></li>
      		</ul>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst info_lst gap-column-important-m">
      			<li><strong>필드 유형</strong><p>이름, 이메일, 전화, 주소, 신용카드 등 표준 힌트가 있는 항목에 모두 매핑했는가?</p></li>
      			<li><strong>하이브리드</strong><p>WebView와 네이티브 폼의 힌트 체계가 동일한 의미로 연결되는가?</p></li>
      			<li><strong>보이는 레이블</strong><p>시각 레이블과 자동완성 힌트가 서로 모순되지 않는가?</p></li>
      			<li><strong>민감 정보</strong><p>자동완성을 끌 수 있는 옵션이 필요한 화면에서는 <code>importantForAutofill</code> 등으로 제어했는가?</p></li>
      		</ul>
      	</section>

      	<section className="indent mt_xxl">
      		<h2 className="ml_mn" data-conlist="false">구현 시 참고</h2>
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">Android</h3>
      			<ul className="mt_m dot_lst gap-column-important-sm">
      				<li><code>autofillHints</code>, <code>importantForAutofill</code>로 필드 목적과 자동완성 허용 여부를 지정합니다.</li>
      				<li>커스텀 뷰는 Autofill 서비스와 호환되는 노드 구조인지 확인합니다.</li>
      			</ul>
      		</article>
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">iOS</h3>
      			<ul className="mt_m dot_lst gap-column-important-sm">
      				<li><code>textContentType</code>(예: <code>.emailAddress</code>, <code>.telephoneNumber</code>, <code>.oneTimeCode</code>)을 지정합니다.</li>
      				<li>비밀번호 필드는 <code>textContentType</code>과 보안 텍스트 입력 설정을 함께 맞춥니다.</li>
      			</ul>
      		</article>
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">WebView</h3>
      			<p className="mt_m">HTML <code>autocomplete</code> 속성을 네이티브 힌트와 동일한 의미로 유지합니다.</p>
      		</article>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst gap-column-important-sm">
      			<li>실제 기기에서 자동완성·비밀번호 저장·OTP 제안이 기대대로 뜨는지 주요 폼을 검증합니다.</li>
      			<li>스크린 리더로 필드에 들어갈 때 이름·힌트·키보드 유형이 일관되는지 확인합니다.</li>
      		</ul>
      	</section>
      <section className="indent mt_xxl">
      	...
      	<p className="mt_l">
      		아래 코드는 <b></b> 검사항목을 구현할 때 바로 참고할 수 있는 기본 예시입니다.
      	...
      	<article className="mt_xl indent">
      		...
      		<div className="mt_l indent">
      			<h4 className="ml_mn">iOS (Swift)</h4>
      			
      <CodeBlock title="인식의 용이성 - iOS (Swift)" language="html" className="uk_gist_code_box">
      {`import UIKit

      final class PerceivableViewController: UIViewController {
      	{% if pageLayout.mainClass == "audio-description" %}
      	func playVideo() { /* 화면해설 트랙 선택 UI 제공 */ }
      	{% elif pageLayout.mainClass == "table-structure" %}
      	func buildTableHeader() { /* 헤더-셀 관계를 접근성으로 노출 */ }
      	{% elif pageLayout.mainClass == "linear-structure" %}
      	func enforceReadingOrder() { view.accessibilityElements = [titleLabel, bodyLabel, actionButton] }
      	{% elif pageLayout.mainClass == "multisensory-support" %}
      	func showError() { /* 색상 + 아이콘 + 텍스트 동시 제공 */ }
      	{% elif pageLayout.mainClass == "orientation" %}
      	override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {}
      	{% elif pageLayout.mainClass == "identify-input-purpose" %}
      	func configureField(_ field: UITextField) { field.textContentType = .emailAddress }
      	{% elif pageLayout.mainClass == "audio-control" %}
      	func setupPlayer(_ player: AVPlayer) { player.isMuted = true /* 사용자가 재생 */ }
      	{% elif pageLayout.mainClass == "separation-of-content" %}
      	func buildGroup() { /* 제목/여백/구분선으로 영역 구분 */ }
      	{% elif pageLayout.mainClass == "image-of-text" %}
      	func useRealText() { titleLabel.text = "실제 텍스트 사용" }
      	{% elif pageLayout.mainClass == "reflow" %}
      	func allowWrap(_ label: UILabel) { label.numberOfLines = 0; label.adjustsFontForContentSizeCategory = true }
      	{% elif pageLayout.mainClass == "text-spacing" %}
      	func setParagraphStyle(_ label: UILabel) { /* lineSpacing 확대 대응 */ }
      	{% endif %}
      }`}
      </CodeBlock>
      ...
      <CodeBlock title="인식의 용이성 - Android" language="html" className="uk_gist_code_box">
      {`{% if pageLayout.mainClass == "identify-input-purpose" %}
      editText.setAutofillHints(View.AUTOFILL_HINT_EMAIL_ADDRESS)
      {% elif pageLayout.mainClass == "audio-control" %}
      playerView.player?.volume = 0f // 기본 음소거
      {% elif pageLayout.mainClass == "linear-structure" %}
      root.accessibilityTraversalAfter = R.id.headerTitle
      {% elif pageLayout.mainClass == "image-of-text" %}
      imageView.contentDescription = "배너 문구: 신규 가입 이벤트"
      {% elif pageLayout.mainClass == "reflow" %}
      ...
      {% endif %}`}
      	...
      	<article className="mt_xl indent">
      		...
      		<div className="mt_l indent">
      			<h4 className="ml_mn">html</h4>
      			
      <CodeBlock title="인식의 용이성 - html" language="html" className="uk_gist_code_box">
      {`{% if pageLayout.mainClass == "table-structure" %}
      <table>
      	<caption>월별 이용량</caption>
      	<thead>
      		<tr><th scope="col">월</th><th scope="col">이용량</th></tr>
      	</thead>
      	<tbody>
      		<tr><th scope="row">7월</th><td>120</td></tr>
      	</tbody>
      </table>
      {% elif pageLayout.mainClass == "audio-description" %}
      <video controls><track kind="descriptions" src="/media/ad.vtt" srclang="ko" /></video>
      {% elif pageLayout.mainClass == "orientation" %}
      ...
      {% elif pageLayout.mainClass == "image-of-text" %}
      <img src="/banner.png" alt="신규 가입 이벤트 10% 할인" />
      {% elif pageLayout.mainClass == "reflow" %}
      <main style="max-width: 100%; overflow-wrap: anywhere;"></main>
      {% elif pageLayout.mainClass == "text-spacing" %}
      <p style="line-height:1.7; letter-spacing:.05em;">가독성 테스트 문장</p>
      {% else %}
      <section aria-label="{{ topic }}"></section>
      {% endif %}`}
      </CodeBlock>
      		...
      		<div className="mt_l indent">
      			<h4 className="ml_mn">Vue</h4>
      			
      <CodeBlock title="인식의 용이성 - Vue" language="html" className="uk_gist_code_box">
      {`<template>
      	{% if pageLayout.mainClass == "identify-input-purpose" %}
      	<input autocomplete="email" />
      	{% elif pageLayout.mainClass == "audio-control" %}
      	...
      	{% elif pageLayout.mainClass == "image-of-text" %}
      	<img :src="banner" alt="신규 가입 이벤트 10% 할인" />
      	{% elif pageLayout.mainClass == "text-spacing" %}
      	<p :style="{ lineHeight: 1.7, letterSpacing: '0.05em' }">가독성 문장</p>
      	{% else %}
      	<section aria-label="{{ topic }}"></section>
      	{% endif %}
      </template>
      ...
      const banner = "/assets/banner.png";
      		...
      		<div className="mt_l indent">
      			<h4 className="ml_mn">React</h4>
      			
      <CodeBlock title="인식의 용이성 - React" language="html" className="uk_gist_code_box">
      {`export default function PerceivableExample() {
      	{% if pageLayout.mainClass == "identify-input-purpose" %}
      	return <input autoComplete="email" />;
      	{% elif pageLayout.mainClass == "audio-control" %}
      	return <video controls muted />;
      	{% elif pageLayout.mainClass == "image-of-text" %}
      	return <img src="/assets/banner.png" alt="신규 가입 이벤트 10% 할인" />;
      	{% elif pageLayout.mainClass == "text-spacing" %}
      	const textStyle = { lineHeight: 1.7, letterSpacing: "0.05em" };
      	return <p style={textStyle}>가독성 문장</p>;
      	{% else %}
      	return <section aria-label="{{ topic }}" />;
      	{% endif %}
      }`}
      </CodeBlock>
      ...
      <footer className="reference_box">
      	<strong className="tit">입력 목적 식별 | 모바일 접근성 콘텐츠 제작 기법(&quot;인식의 용이성(Perceivable)&quot;) 참조</strong>
...
export default IdentifyInputPurposePage
