...
const PAGE_DATA = {
  title: "상태 메시지 | 모바일 접근성 콘텐츠 제작 기법(\"견고성(Robust)\")",
  description: "모바일 접근성 콘텐츠 제작 기법이란 장애 유무에 관계없이 모든 사용자가 모바일 콘텐츠를 쉽게 이해하고 이용할 수 있도록 하는 설계하는 방법입니다. 이 페이지에서는 MACAG ‘상태 메시지’에 따라, 조회 결과·오류·로딩 등 변화를 보조 기술에 즉시 알리는 방법을 정리합니다.",
  keyword: "Accessibility, a11y, macag, WCAG, 상태 메시지, live region, Announcement, AccessibilityEvent, 견고성",
}

function StatusMessagesPage() {
  ...
  return (
    <>
      	...
      	<blockquote className="uk_note mt_xxl" role="note">
      		...
      		<p>
      			<b className="t_black">관련 지침(보조기술과의 호환성) : 조회 결과·장바구니 수량 변경·로딩·폼 오류 등 중요한 상태 변화가 초점 이동 없이도 보조 기술 사용자에게 전달되는가?</b><br />
      		...
      		<p className="mt_ms reference">
      			...
      			<a href="https://www.w3.org/WAI/WCAG22/quickref/#status-messages" title="새창 열림" target="_blank" rel="noreferrer" className="fas weight-500">WCAG 2.2 — Status Messages</a>
      		</p>
      	</blockquote>
      	...
      	<section className="indent mt_xxl">
      		...
      		<p className="mt_l">
      			폼 전송 실패, 네트워크 오류, 비동기 로딩 완료 등은 사용자의 다음 행동을 결정합니다.
      			포커스를 옮기지 않고도 음성으로 즉시 알려 주어야 동등한 경험이 됩니다.
      		</p>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst info_lst gap-column-important-m">
      			<li><strong>스크린 리더 사용자</strong><p>화면 중앙 토스트를 시각적으로 보지 못하는 사용자.</p></li>
      			<li><strong>저시력 사용자</strong><p>짧게 사라지는 배너를 놓칠 수 있는 사용자.</p></li>
      			<li><strong>스위치·키보드 사용자</strong><p>포커스가 입력란에 고정된 채 비동기 결과만 바뀌는 흐름을 쓰는 사용자.</p></li>
      			<li><strong>전체 사용자</strong><p>멀티태스킹 중 알림 센터만 보고 상태를 확인하려는 사용자.</p></li>
      		</ul>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst info_lst gap-column-important-m">
      			<li><strong>성공·실패</strong><p>저장 성공, 결제 실패 등이 음성으로도 요약되는가?</p></li>
      			<li><strong>진행률</strong><p>장시간 작업에 진행 상태가 업데이트될 때마다 적절히 알리는가?</p></li>
      			<li><strong>중복·과다</strong><p>같은 메시지가 초당 여러 번 반복되지 않는가?</p></li>
      			<li><strong>우선순위</strong><p>긴급(차단 오류)과 일반(완료 토스트)을 polite/assertive 등으로 구분하는가?</p></li>
      		</ul>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<article className="mt_l indent">
      			...
      			<ul className="mt_m dot_lst gap-column-important-sm">
      				<li><code>AccessibilityEvent</code>, <code>announceForAccessibility</code>, 적절한 <code>liveRegion</code> 속성을 사용합니다.</li>
      				<li>Snackbar와 함께 라이브 영역이 연결되어 있는지 확인합니다.</li>
      			</ul>
      		</article>
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">iOS</h3>
      			<ul className="mt_m dot_lst gap-column-important-sm">
      				<li><code>UIAccessibility.post(notification:argument:)</code> 등으로 레이아웃 변경·알림을 전달합니다.</li>
      				<li>SwiftUI에서는 변경된 상태에 맞는 접근성 속성 업데이트를 검토합니다.</li>
      			</ul>
      		</article>
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">공통</h3>
      			<p className="mt_m">과도한 연속 알림은 사용자를 피로하게 하므로, 중요도에 따라 polite/assertive를 구분합니다.</p>
      		</article>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst gap-column-important-sm">
      			<li>스크린 리더를 켠 채 네트워크 끊김·복구, 폼 오류, 장바구니 변경을 재현하고 알림 순서를 기록합니다.</li>
      			<li>포커스를 입력 필드에 둔 상태에서만 완료되는 작업(자동 저장 등)이 있는지 확인합니다.</li>
      		</ul>
      	</section>
      <section className="indent mt_xxl">
      	...
      	<p className="mt_l">
      		아래 코드는 <b></b> 검사항목을 충족하기 위해, 보조기술 호환성을 높이는 방식으로 구성한 예시입니다.
      		폼 제출 결과, 저장 완료, 오류 같은 변화가 포커스 이동 없이도 전달되도록 상태 공지 방식을 일관되게 적용해 주세요.
      	</p>
      ...
      <CodeBlock title="견고성 - iOS (Swift)" language="javascript" className="uk_gist_code_box">
      {`import UIKit

      final class RobustViewController: UIViewController {
      ...
      <CodeBlock title="견고성 - Android" language="javascript" className="uk_gist_code_box">
      ...
      <CodeBlock title="견고성 - html" language="javascript" className="uk_gist_code_box">
      {`<p id="status" role="status" aria-live="polite"></p>
      <button onclick="document.getElementById('status').textContent='저장되었습니다.'">저장</button>`}
      </CodeBlock>
      ...
      <CodeBlock title="견고성 - Vue" language="html" className="uk_gist_code_box">
      {`<template>
      	<p role="status" aria-live="polite">{{ status }}</p>
      	<button @click="status='저장되었습니다.'">저장</button>
      </template>`}
      </CodeBlock>
      ...
      <CodeBlock title="견고성 - React" language="javascript" className="uk_gist_code_box">
      {`import { useState } from "react";

      export default function RobustExample() {
      	const [status, setStatus] = useState("");
      	return (
      		<>
      			<p role="status" aria-live="polite">{status}</p>
      			<button onClick={() => setStatus("저장되었습니다.")}>저장</button>
      		</>
      	);
      }`}
      </CodeBlock>
      ...
      <footer className="reference_box">
      	<strong className="tit">상태 메시지 | 모바일 접근성 콘텐츠 제작 기법(&quot;견고성(Robust)&quot;) 참조</strong>
...
export default StatusMessagesPage
