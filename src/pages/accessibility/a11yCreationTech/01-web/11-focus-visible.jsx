...
const PAGE_DATA = {
  title: "초점 이동과 표시 | 웹 접근성 콘텐츠 제작 기법(\"운용의 용이성(Operable)\")",
  description: "웹 접근성 콘텐츠 제작 기법이란 장애 유무에 관계없이 모든 사용자가 웹 콘텐츠를 쉽게 이해하고 이용할 수 있도록 하는 설계하는 방법입니다. 이 페이지에서는 웹 접근성 콘텐츠 제작 기법 중 하나인 '초점 이동과 표시'에 대해 소개합니다.",
  keyword: "Accessibility, a11y, wcag, kwcag, macag, kiosk, 접근성, 웹 콘텐츠 접근성, 한국형 웹 콘텐츠 접근성, 모바일 앱 접근성, 무인정보단밀기 접근성, 웹 접근성, 웹 접근성 콘텐츠 제작 기법, 한국형 웹 콘텐츠 접근성 지침 2.2, WCAG2.2, KWCAG2.2, 보조기술과의호환성, 접근성 테스트 도구 활용 점검방법, 스크린 리더, Jaws, NVDA, 센스리더, Sense Reader, 시각장애인, 지체 장애 사용자, 인지장애 사용자",
}
  ...
  return (
    <>
      	...
      	<blockquote className="uk_note mt_xxl" role="note">
      		...
      		<p>
      			<b className="t_black">관련 지침 : 키보드에 의한 초점은 논리적으로 이동해야 하며, 시각적으로 구별할 수 있어야 한다.</b><br />
      			웹 페이지에서 제공하는 모든 기능은 키보드만으로 사용할 수 있어야 하며, 이때 사용자 입력 간 초점 이동은 논리적이고 적절한 순서를 따라야 합니다.
      			초점 이동 과정에서 콘텐츠가 조작 불가능한 상태가 되거나 예기치 않게 페이지가 전환되어서는 안 됩니다.<br />
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst info_lst gap-column-important-m">
      			<li>
      				...
      				<p>
      					사용자가 키보드를 이용하여 초점을 이동하는 경우,
      					이동 순서가 일반적인 관례를 벗어나면 혼란을 줄 수 있으므로 초점 이동은 사용자가 예측할 수 있는 순서를 따라야 합니다.
      					가장 바람직한 방법은 널리 사용되는 관례를 따르는 방식으로 콘텐츠를 제공하는 것입니다.<br />
      					예를 들어, 로그인 양식에서는 초점이 [<i className="underline">사용자 아이디 → 비밀번호 → 로그인 버튼</i>]의 순서로 이동해야 하며,
      					이는 사용자의 예상을 벗어나지 않는 자연스러운 흐름입니다.<br />
      					만약 불가피하게 관례와 다른 초점 이동 순서를 적용해야 할 경우에는,
      					사용자 입력 간의 초점 이동이 논리적 흐름에 따라 이루어지도록 구현하는 것이 필요합니다.
      				</p>
      			</li>
      			<li>
      				<strong>함정 또는 오류 방지</strong>
      				<p>
      					웹 콘텐츠는 더 이상 키보드 조작이 불가능한 상태로 구현되지 않아야 합니다.
      					즉, 사용자가 초점을 이동하거나 다음 입력으로 이동할 수 없는 상태가 되어서는 안 됩니다.
      					또한, 이전 페이지로 초점을 이동하거나,
      					다른 사용자 입력이나 제어 요소로의 초점 이동이 불가능해지는 상태가 되지 않도록 구현해야 합니다.
      				</p>
      			</li>
      			<li>
      				<strong>초점의 시각화</strong>
      				<p>
      					사용자가 키보드 조작이나 마우스를 사용해 초점을 이동했을 때, 해당 콘트롤이 초점을 받았음을 명확히 시각적으로 구별할 수 있도록 해야 합니다.
      					예를 들어, 키보드를 이용해 버튼을 선택할 때 버튼 주위에 점선 테두리가 나타나는 것과 같이,
      					초점이 맞춰진 요소가 사용자에게 명확히 인식되도록 해야 합니다.<br />
      					또한, 위치 지정 도구(마우스)에 의한 초점과 키보드에 의한 초점 표시 방법이 다를 수 있으므로,
      					각 경우에 맞는 적절한 시각적 표시를 제공할 수 있습니다.
      					예를 들어, 마우스로 클릭한 요소는 다른 방식으로 강조될 수 있으며, 키보드로 이동한 요소는 다른 스타일로 강조될 수 있습니다.<br />
      					초점의 시각적 표시에는 시간 제한이 없어야 하며, 사용자가 초점을 맞추었을 때 그 표시가 충분히 명확하고 지속적으로 유지될 수 있도록 해야 합니다.
      					이는 저시력 사용자나 키보드를 사용하는 사용자에게 특히 중요합니다.
      				</p>
      			</li>
      		</ul>
      		<p className="mt_ml success_criteria">
      			<a href="https://www.w3.org/TR/WCAG22/#focus-order" title="새창 열림" target="_blank" rel="noreferrer" className="fas weight-500 box_link external before_none">2.4.3 포커스 순서 (Focus Order) (Level A)</a>
      			...
      			<a href="https://www.w3.org/WAI/WCAG22/Understanding/focus-order.html" title="새창 열림" target="_blank" rel="noreferrer" className="fas weight-500 box_link external before_none">WAI - Understanding Focus</a>
      			<a href="https://developer.mozilla.org/ko/docs/Web/CSS/outline" title="새창 열림" target="_blank" rel="noreferrer" className="fas weight-500 box_link external before_none">MDN - CSS outline 속성</a>
      		</p>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst gap-column-important-sm">
      			<li>
      				화면낭독프로그램을 이용하는 사용자의 경우, 사용자 입력 주변의 상하좌우에 위치한 콘텐츠에 대한 정보를 알 수 없습니다.
      				따라서 웹 콘텐츠를 사용하는 과정에서 키보드 조작에 의한 사용자 입력 간의 이동 순서는 관례를 따라야 합니다.
      				그렇지 않으면 사용자 입력의 조작 과정에서 혼란을 주게 됩니다.
      			</li>
      			<li>
      				마우스나 키보드 조작을 통해 특정 영역으로 콘트롤을 이동하였을 경우,
      				해당 영역이 초점을 받았음을 시각적으로 알려주면 저시력장애인, 고령자, 지체장애인뿐만 아니라
      				비장애인들도 현재 초점이 어느 콘트롤에 위치하고 있는지 확인할 수 있고, 콘트롤의 기능을 활성화시킬 수 있는지 쉽게 인지할 수 있습니다.
      			</li>
      		</ul>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<p className="mt_l">
      			키보드로 웹 페이지를 탐색할 때, 포커스가 <b>논리적인 순서로 이동</b>하고,
      			사용자가 현재 위치를 <b>시각적으로 인지할 수 있도록 표시(Outline 등)</b> 되어야 합니다.
      			포커스가 보이지 않거나, 순서가 뒤섞이면 혼란을 초래할 수 있습니다.
      		</p>
      		...
      		<div className="table_summary value_200 mt_l">
      			...
      			<ul className="lst_bd">
      				<li>
      					...
      					<p className="cell content">스크린 리더와 포커스 탐색 병행</p>
      				</li>
      				<li>
      					<p className="cell value">지체 장애 사용자</p>
      					<p className="cell content">키보드 전용 조작</p>
      				</li>
      				<li>
      					<p className="cell value">인지장애 사용자</p>
      					<p className="cell content">시각적 피드백이 필요</p>
      				</li>
      			</ul>
      		</div>
      	</section>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst gap-column-important-sm">
      			<li>
      				<i className="key_tab">tab</i> 키로 요소를 탐색할 때 논리적 순서로 이동하는가?
      			</li>
      			<li>
      				현재 포커스된 요소가 시각적으로 구분 가능한가?
      			</li>
      			<li>
      				포커스 <code>outline</code> 제거 없이 사용자 정의 스타일을 제공하는가?
      			</li>
      			<li>
      	...
      	<section className="indent mt_xxl">
      		...
      		<ul className="mt_l dot_lst gap-column-important-sm">
      			<li>
      				<i className="key_tab">tab</i> 키만으로 전체 페이지 탐색을 확인합니다.
      			</li>
      			<li>
      				포커스 시각적 표시(<code>outline</code>, <code>border</code> 등)를 확인합니다.
      			</li>
      			<li>
      	...
      	<section className="indent mt_xxl">
      		...
      		<article className="mt_l indent">
      			<h3 className="ml_mn t_blue">html 예시 – 포커스 스타일 제공</h3>

      			
      <CodeBlock title="html - 초점 이동과 표시" language="javascript" className="uk_gist_code_box">
      {`<!-- html 예시 -->
      		...
      		<article className="mt_xl indent">
      			<h3 className="ml_mn t_blue">Vue 예시 – 사용자 정의 포커스 스타일</h3>

      			
      <CodeBlock title="Vue - 초점 이동과 표시" language="javascript" className="uk_gist_code_box">
      {`<template>
      		...
      		<article className="mt_xl indent">
      			<h3 className="ml_mn t_blue">React 예시 – 논리적 포커스 순서</h3>

      			
      <CodeBlock title="React - 초점 이동과 표시" language="javascript" className="uk_gist_code_box">
      {`function LoginForm() {
      						return (
      							<>
      	...
      	<section className="indent mt_xxl">
      		<h2 className="ml_mn" data-conlist="false">점검 기준</h2>
      			...
      			<li>
      				<code>outline</code> 제거 시 대체 스타일이 제공되는가?
      			</li>
      		</ul>
      	</section>
      ...
      <CodeBlock title="초점 이동과 표시 - 미준수 사례" language="javascript" className="uk_gist_code_box">
      {`<button onclick="alert('버튼 클릭!')">클릭</button>
      					<div onclick="alert('Div 클릭!')">클릭할 수 있는 div</div>`}
      </CodeBlock>
      			...
      			<dl className="dl_dot_lst mt_ms">
      				...
      				<dd></dd>
      				<dd>
      					<code>&lt;div&gt;</code> 요소에 클릭 이벤트가 적용되어 있지만,
      					기본적으로 키보드 초점을 받을 수 없기 때문에 키보드 사용자는 <code>&lt;div&gt;</code> 요소에 접근할 수 없습니다.
      				</dd>
      				<dd>
      					<code>&lt;button&gt;</code> 요소는 초점을 받을 수 있지만, <code>&lt;div&gt;</code>는 클릭할 수 있더라도 초점 이동이 불가능하여
      					화면 리더 사용자나 키보드 사용자가 그 요소에 접근하지 못합니다.
      				</dd>
      				<dd>
      ...
      <CodeBlock title="초점 이동과 표시 - 준수 사례" language="javascript" className="uk_gist_code_box">
      {`<button onclick="alert('버튼 클릭!')">클릭</button>
      					<div tabindex="0" onclick="alert('Div 클릭!')">클릭할 수 있는 div</div>`}
      </CodeBlock>
      			...
      			<dl className="dl_dot_lst mt_ms">
      				...
      				<dd></dd>
      				<dd>
      					<code>&lt;div&gt;</code> 요소에 <code>tabindex=&quot;0&quot;</code> 속성을 추가하여 키보드 초점이 가능해졌습니다.
      					키보드 사용자도 이 요소를 탐색할 수 있습니다.
      				</dd>
      				<dd>
      					<code>&lt;button&gt;</code> 요소는 기본적으로 키보드 초점을 받을 수 있으며,
      					버튼을 클릭할 때와 키보드로 활성화할 때 동일한 동작을 보장합니다.
      				</dd>
      				<dd>
      					초점 이동 시, 시각적으로도 초점이 있는 요소가 강조되거나 하이라이트되어야 합니다.
      					예를 들어 CSS에서 <code>:focus</code> 상태를 이용하여 스타일을 적용할 수 있습니다.
      				</dd>
      			</dl>
      		</article>
      	...
      	<section className="indent mt_xxl">
      		...
      		<div className="mt_xl youtube_box" style={{ '--padding-top': "56.25%" }}>
      			<iframe width="560" height="315" src="https://www.youtube.com/embed/b1hALv6l9ck?si=Z95EnyuZ0-JWyHlr" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
      		</div>
      		<p className="mt_ms">
      ...
      <footer className="reference_box">
      	<strong className="tit">초점 이동과 표시 | 웹 접근성 콘텐츠 제작 기법(&quot;운용의 용이성(Operable)&quot;) 참조</strong>
