#!/usr/bin/env node
/**
 * Convert legacy accessibility Nunjucks pages to React JSX.
 * Usage: node scripts/migrate-accessibility-njk.mjs [--skip=slug] [path-filter...]
 */
...
const AS_IS = path.join(ROOT, 'src/as-is/accessibility')
const PAGES = path.join(ROOT, 'src/pages/accessibility')
...
function walkNjk(dir, list = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (entry.name.endsWith('.njk') && !entry.name.startsWith('_')) list.push(full)
  }
  return list
}
...
function extractBlock(njk, blockName) {
  const re = new RegExp(`\\{%\\s*block ${blockName}\\s*%\\}([\\s\\S]*?)\\{%-?\\s*endblock\\s*-?%\\}`, 'i')
...
const PATH_IMG_MAP = {
  img_a11y: 'IMG.A11Y',
  img_wai_aria: 'IMG.WAI_ARIA',
  img_a11y_tech: 'IMG.A11Y_TECH',
  img_a11y_tech_common: 'IMG.A11Y_TECH_COMMON',
  img_a11y_tech_kiosk: 'IMG.A11Y_TECH_KIOSK',
  img_a11y_tech_m_p: 'IMG.A11Y_TECH_M_P',
  img_a11y_tech_m_p_51: 'IMG.A11Y_TECH_M_P_51',
  img_a11y_tech_m_p_52: 'IMG.A11Y_TECH_M_P_52',
  img_a11y_tech_m_p_53: 'IMG.A11Y_TECH_M_P_53',
  img_a11y_tech_m_p_54: 'IMG.A11Y_TECH_M_P_54',
  img_a11y_tech_m_p_55: 'IMG.A11Y_TECH_M_P_55',
  img_a11y_tech_w_o: 'IMG.A11Y_TECH_W_O',
  img_a11y_tech_w_u: 'IMG.A11Y_TECH_W_U',
}

function extractPageNumber(njk) {
  const match = njk.match(/\{%\s*set\s+pageNumber\s*=\s*\{([^}]+)\}/)
  if (!match) return {}
  const block = match[1]
  const result = {}
  const numRe = /(\w+):\s*(\d+)/g
  let m
  while ((m = numRe.exec(block)) !== null) result[m[1]] = Number(m[2])
  return result
}

function extractTechSubTitle(njk) {
  const m = njk.match(/\{%\s*set\s+techSubTitle\s*=\s*"([^"]+)"/)
  return m?.[1] ?? ''
}

function resolveTitle(title, techSubTitle) {
  return String(title ?? '').replace(/~techSubTitle~/g, techSubTitle)
}
...
function resolveIncludes(content, njkPath, pageNumber) {
  let out = content
  const includeRe = /\{%\s*include\s+"accessibility\/([^"]+)"\s*%\}/g
  out = out.replace(includeRe, (_, rel) => {
    const includePath = path.join(AS_IS, rel)
    if (rel.includes('_dev-method-common.njk')) {
      const depth3 = pageNumber.depth3
      const map = {
        5: '_dev-method-perceivable.njk',
        6: '_dev-method-operable.njk',
        7: '_dev-method-understandable.njk',
        8: '_dev-method-robust.njk',
      }
      const file = map[depth3] ?? '_dev-method-perceivable.njk'
      return readPartialContent(path.join(path.dirname(includePath), file))
    }
    return readPartialContent(includePath)
  })
  return out
}
...
function resolveLayoutPartials(content, njkDir, layout) {
  ...
  if (layout.partReference) {
    suffix = `\n\n${readPartialContent(path.join(njkDir, layout.partReference))}`
  } else if (layout.reference === true) {
    const refPath = path.join(njkDir, '_reference.njk')
    if (fs.existsSync(refPath)) suffix = `\n\n${readPartialContent(refPath)}`
  }
  if (layout.d2BottomCommon) {
    const bottomPath = path.join(AS_IS, 'a11yCreationTech', layout.d2BottomCommon)
    suffix += `\n\n${readPartialContent(bottomPath)}`
  }
...
function convertViewEditorBlocks(html) {
  return html.replace(
    /<div\s+class="view_editor([^"]*)"([^>]*data-target="([^"]+)"[^>]*)>\s*<div\s+class="uk_editor([^"]*)"([^>]*)>\s*<div\s+class="code_wrap">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/gi,
    (_full, viewClass, _vAttrs, target, editorClass, editorAttrs) => {
      ...
      const wrapClass = ['view_editor', viewClass.trim()].filter(Boolean).join(' ')
...
function convertHtmlChunk(html, pageTitle) {
  ...
  out = out.replace(/\{\{[^}]+\}\}/g, '')
  out = out.replace(/\bsrc="\$\{(IMG\.[A-Z0-9_]+)\}\/([^"]+)"/g, 'src={`${$1}/$2`}')
...
function convertContent(rawContent, pageTitle) {
  const withoutComments = stripNjkComments(rawContent)
...
function buildLayout(pageLayout) {
  const layout = {
    mainClass: pageLayout.mainClass ?? 'a11y_page',
    type: 'accessibility',
    titleSticky: pageLayout.titleSticky !== false,
  ...
  if (pageLayout.sideMenuSticky === true) layout.sideMenuSticky = true
  return layout
}

function buildPage({ componentName, pageData, pageLayout, content, isPartial }) {
  ...
  const needsImg = /\bIMG\.[A-Z0-9_]+\b/.test(content)

  const imports = []
  if (!isPartial) {
    imports.push("import { useAccessibilityPageMeta } from '@/hooks/useAccessibilityPageMeta.js'")
  }
...
`
  ...
  return `${imports.join('\n')}
${metaBlock}
function ${componentName}() {${hookBlock}
...
function migrateOne(njkPath) {
  ...
  const rel = path.relative(AS_IS, njkPath)
  if (filters.length && !filters.some((f) => rel.includes(f))) return

  const jsxPath = path.join(PAGES, rel.replace(/\.njk$/, '.jsx'))
  ...
  const pageNumber = extractPageNumber(njk)
  const techSubTitle = extractTechSubTitle(njk)
  const isPartial = !njk.includes('pageData')

  if (pageData.title) pageData.title = resolveTitle(pageData.title, techSubTitle)
  ...
  rawContent = resolveIncludes(rawContent, njkPath, pageNumber)
  if (!isPartial) rawContent = resolveLayoutPartials(rawContent, njkDir, pageLayout)

  const content = convertContent(rawContent, pageData.title ?? '')
  const componentName = toComponentName(slug)
  const page = buildPage({ componentName, pageData, pageLayout, content, isPartial })
...
console.log(`Done. ${all.length} files processed.`)
